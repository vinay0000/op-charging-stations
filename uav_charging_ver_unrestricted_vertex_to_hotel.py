...
# limit the same vertex-to-vertex transitions to atmost 1.
# The same **vertex-to-hotel** and hotel-to-vertex and hotel-to-hotel transitions are *not* limited. 
# This condition differs from the original OPHS formulation in that, the original formulation restricts same vertex-to-hotel transitions to atmost one.
for i in range(H, H+N):
    m.addCons(scip.quicksum(x[i][j][d]  for d in range(0, D) for j in range(H, H+N)) <=1) 
