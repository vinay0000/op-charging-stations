"""
 Divsalar, A., Vansteenwegen, P., & Cattrysse, D. (2013). A variable neighborhood search method for the orienteering problem 
    ...
    In ver-3 the objective includes the scores for the transits between hotels and vertices. The score can be non-zero.
    Another difference wrt ver-2 is the addition of a redundant constraint to (hopefully) improve converegence speed.
    The redundant constraint is to: "restrict same hotel-to-vertex transitions".
"""
...
def read_data_file(file_path):
    with open(file_path, 'r') as file:
        """
            Refer to the README file in the data/OPHS instances_February 2013 folder.
                ...
                TTd	= trip length for each trip d. This is not used, intead
    ...
    return N, H, D, T_Max, T_CH, location, Si


#file_path = os.path.join(current_dir, 'data/dataSet')
file_path = os.path.join(current_dir, 'data/science_data/Fracking_25_scival_N24_H1_D10_Tmax28800_Tch1800.ophs')
...
result_file_name = f"results/data_objects_{parts[-2].replace(' ', '_')}_{parts[-1].replace('ophs', 'pkl')}"
#result_file_name = "dataSet.pkl"
result_fp = os.path.join(current_dir, result_file_name)
print(result_file_name)

N, H, D, T_Max, T_CH, c_pos, Si = read_data_file(file_path)
